class Store {
    ...
    storeUpdate(){
        ...
        if(this.keyword !== ""){
            ...
            viewList = viewList.filter(item => regex.test(item.product_name) || regex.test(item.brand))
                .map(item => {
                    item.product_name = item.product_name.replace(regex, m1 => `<span class="bg-gold">${m1}</span>`);
                    item.brand = item.brand.replace(regex, m1 => `<span class="bg-gold">${m1}</span>`);
                    return item;
                });
        }
        ...
        if(viewList.length > 0){
            ...
            this.$storeList.html(`<div class="w-100 py-5 fx-n2 py-5 text-muted text-center">일치하는 상품이 없습니다.</div>`);
        }
    }
    ...
    cartUpdate(){
        if(this.cartList.length > 0){
            ...
            this.$cartList.html(`<div class="w-100 py-5 fx-n2 py-5 text-muted text-center">장바구니에 담긴 상품이 없습니다.</div>`);
        }
        ...
        $("#buy-modal form").on("submit", e => {
            ...
            let viewList = [
                ...this.cartList.map(item => {
                    let text = `${item.json.product_name}           ${item.json.price.toLocaleString()}원 × ${item.buyCount.toLocaleString()}개 = ${item.totalPrice.toLocaleString()}원`;
            ...
            canvas.height = viewList.length * (TEXT_GAP + TEXT_SIZE) + PADDING * 2;
            ...
            viewList.forEach(({text}, i) => {
                ctx.fillText(text, PADDING, PADDING + TEXT_SIZE * (i+1) + TEXT_GAP * i);
            });
